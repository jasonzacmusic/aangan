...
- **Fleet card** (on Home) — the six ESP32 boards, live up/down, from `/api/fleet` on the LAN server.
...
- **Install & test** — a phone-friendly breadboard map for the six room boards, six commissioning checkpoints, electrical boundaries, and saved pass/fail progress.
...
| GET | `/api/fleet` | — | `FleetDevice[]` (the six ESP32s; LAN server pings each board) |
